...
namespace CIPHER.Helpers
{
    public static class SessionManager
    {
        ...
        public static void RefreshUser()
        {
            ...
            var cmd = new SqlCommand("SELECT XP, CryptCoin, Rank, AccountStatus, XPBoostMissions FROM Users WHERE UserID=@id", conn);
            cmd.Parameters.AddWithValue("@id", CurrentUser.UserID);
            using var r = cmd.ExecuteReader();
            if (!r.Read()) return;
            CurrentUser.XP = (int)r["XP"];
            CurrentUser.CryptCoin = (int)r["CryptCoin"];
            CurrentUser.XPBoostMissions = (int)r["XPBoostMissions"];
